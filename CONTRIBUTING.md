...
--------------------------
Adding a new external base type and lifting it into a temporal type (as `tcbuffer`, `tpose`, `tnpoint`, and `trgeometry` are) touches the catalog, the bbox dispatch, the indexes, the datagen, the regression suite, and the DocBook chapter. A dedicated guide walks through the design decisions, the implementation order, the parity checklist every type must satisfy, and the production-readiness pass that turns a working type into one production workloads can rely on. Read it before opening any code: [doc/contributing/new_temporal_type.md](doc/contributing/new_temporal_type.md).
