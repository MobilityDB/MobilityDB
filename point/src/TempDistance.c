...
PG_FUNCTION_INFO_V1(distance_tpoint_tpoint_old);

PGDLLEXPORT Datum
distance_tpoint_tpoint_old(PG_FUNCTION_ARGS)
{
	...
	Datum (*operator)(Datum, Datum);
	if (sync1->valuetypid == type_oid(T_GEOMETRY))
	{
		if (MOBDB_FLAGS_GET_Z(sync1->flags) && MOBDB_FLAGS_GET_Z(sync2->flags))
			operator = &geom_distance3d;
		else
			operator = &geom_distance2d;
	}
