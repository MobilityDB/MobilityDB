 ...
 *
 * TInstant has no embedded bbox so a simple copy + tag rewrite is safe.
 * TSequence and TSequenceSet DO embed a bbox whose size and layout differ
 * between tbigint (TBox) and th3index (STBox).  We therefore rebuild
 * those subtypes from their instants so the bbox is recomputed correctly
 * for the target type.
 *****************************************************************************/
...
tbigint_to_th3index(const Temporal *temp)
{
  ...
  {
    /* TInstant: no embedded bbox — tag rewrite is sufficient */
  ...
  if (temp->subtype == TSEQUENCE)
  {
    ...
    for (int i = 0; i < seq->count; i++)
    {
      ...
      instants[i] = tinstant_make(tinstant_value_p(inst), T_TH3INDEX, inst->t);
    }
  ...
  for (int i = 0; i < ss->count; i++)
  {
    ...
    for (int j = 0; j < seq->count; j++)
    {
      ...
      instants[j] = tinstant_make(tinstant_value_p(inst), T_TH3INDEX, inst->t);
    }
    sequences[i] = tsequence_make_free(instants, seq->count,
      seq->period.lower_inc, seq->period.upper_inc,
      MEOS_FLAGS_GET_INTERP(seq->flags), NORMALIZE_NO);
  }
  return (Temporal *) tsequenceset_make_free(sequences, ss->count, NORMALIZE_NO);
}
...
th3index_to_tbigint(const Temporal *temp)
{
  ...
  if (temp->subtype == TSEQUENCE)
  {
    ...
    {
      const TInstant *inst = TSEQUENCE_INST_N(seq, i);
      instants[i] = tinstant_make(tinstant_value_p(inst), T_TBIGINT, inst->t);
    }
    return (Temporal *) tsequence_make_free(instants, seq->count,
      seq->period.lower_inc, seq->period.upper_inc,
      MEOS_FLAGS_GET_INTERP(seq->flags), NORMALIZE_NO);
  ...
  for (int i = 0; i < ss->count; i++)
  {
    ...
    {
      const TInstant *inst = TSEQUENCE_INST_N(seq, j);
      instants[j] = tinstant_make(tinstant_value_p(inst), T_TBIGINT, inst->t);
    }
    sequences[i] = tsequence_make_free(instants, seq->count,
      seq->period.lower_inc, seq->period.upper_inc,
      MEOS_FLAGS_GET_INTERP(seq->flags), NORMALIZE_NO);
