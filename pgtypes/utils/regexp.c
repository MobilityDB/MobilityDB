...
// MEOS added it to redefine PG_GET_COLLATION() with DEFAULT_COLLATION_OID
...
typedef struct regexp_matches_ctx
{
  ...
  /* workspace for pg_build_regexp_match_result() */
...
static text **pg_build_regexp_match_result(regexp_matches_ctx *matchctx, int *count);
// static text *pg_build_regexp_split_result(regexp_matches_ctx *splitctx);
 ...
 */
text *
text_regexsubstr(text *s, text *p)
{
  regex_t *re;
  regmatch_t pmatch[2];
  int so, eo;
  ...
  return pg_text_substr(s, so + 1, eo - so);
}
 ...
 */
text *
text_regexreplace_noopt(text *s, text *p, text *r)
{
 ...
 */
text *
text_regexreplace(text *s, text *p, text *r, text *opt)
{
  /*
 ...
 */
text *
text_regexreplace_extended(text *s, text *p, text *r, int start, int n,
  text *flags)
{
 ...
 *
 * pg_similar_escape() is the common workhorse for three SQL-exposed
 ...
 * MEOS: Derived from static function similar_escape_internal
 */
text *
pg_similar_escape(text *pat_text, text *esc_text)
{
  text *result;
  char *p, *e, *r;
  int plen, elen;
  ...
  text *flags, int subexpr)
{
  ...
  {
    meos_error(ERROR, MEOS_ERR_INVALID_ARG_VALUE,
      "invalid value for parameter \"endoption\": %d", endoption);
    return -1;
  }
  if (subexpr < 0)
  {
 ...
 */
text **
pg_regexp_match(text *orig_str, text *pattern, text *flags, int *count)
{
  pg_re_flags re_flags;
  regexp_matches_ctx *matchctx;
  ...
  /* Create workspace that pg_build_regexp_match_result needs */
  matchctx->elems = (Datum *) palloc(sizeof(Datum) * matchctx->npatterns);
  matchctx->nulls = (bool *) palloc(sizeof(bool) * matchctx->npatterns);

  return pg_build_regexp_match_result(matchctx, count);
}
...
// {
  ...
  // pg_re_flags re_flags;
    ...
    // /* Pre-create workspace that pg_build_regexp_match_result needs */
    // matchctx->elems = (Datum *) palloc(sizeof(Datum) * matchctx->npatterns);
    // matchctx->nulls = (bool *) palloc(sizeof(bool) * matchctx->npatterns);
  ...
  // {
    // ArrayType *result_ary = pg_build_regexp_match_result(matchctx);
    // matchctx->next_match++;
...
/*
 * pg_build_regexp_match_result - build output array for current match
 * MEOS: Derived from PostgreSQL function build_regexp_match_result by
 * returning a text ** instead of an ArrayType *
 */
static text **
pg_build_regexp_match_result(regexp_matches_ctx *matchctx, int *count)
{
  assert(matchctx); assert(count);
  ...
  for (i = 0; i < matchctx->npatterns; i++)
  {
    ...
    else if (buf)
    {
      ...
      elems[count1++] = PointerGetDatum(pg_cstring_to_text_with_len(buf, len));
    }
    else
      elems[count1++] = PointerGetDatum(pg_text_substr(matchctx->orig_str,
        so + 1, eo - so));
  }

  /* Construct the result and return */
  text **result = palloc(sizeof(text *) * count1);
  for (i = 0; i < count1; i++)
    result[i] = DatumGetTextP(elems[i]);
  *count = count1;
  return result;
}
...
// {
  ...
  // {
    // Datum result = pg_build_regexp_split_result(splitctx);
    // splitctx->next_match++;
...
// {
  ...
  // {
    // astate = accumArrayResult(astate, pg_build_regexp_split_result(splitctx),
...
/*
 * pg_build_regexp_split_result - build output string for current match
 *
 ...
 */
// static text *
// pg_build_regexp_split_result(regexp_matches_ctx *splitctx)
// {
  ...
  int subexpr)
{
  ...
  return pg_text_substr(matchctx->orig_str, so + 1, eo - so);
}
