 ...
 */
static void
traverse_lacons(struct cnfa *cnfa, int st, int *arcs_count, regex_arc_t *arcs,
  int arcs_len)
{
  struct carc *ca;
 ...
 */
void
pg_reg_getoutarcs(const regex_t *regex, int st, regex_arc_t *arcs,
  int arcs_len)
{
  struct cnfa *cnfa;
  int arcs_count;
...
pg_reg_getnumcharacters(const regex_t *regex, int co)
{
  ...
  if (co <= 0 || (size_t) co > cm->max)  /* <= 0 rejects WHITE and RAINBOW */
    return -1;
 ...
 */
void
pg_reg_getcharacters(const regex_t *regex, int co, pg_wchar *chars,
  int chars_len)
{
  struct colormap *cm;
  chr c;
  ...
  if (co <= 0 || (size_t) co > cm->max || chars_len <= 0)
    return;
