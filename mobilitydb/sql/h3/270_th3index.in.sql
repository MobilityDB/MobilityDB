 ...
 *
 * th3index and tbigint share the same int64 payload layout, but the
 * Temporal header's temptype field differs (T_TH3INDEX vs T_TBIGINT).
 * The cast functions rewrite that one field so that dispatcher logic
 * (tspatial_type, tspatial_set_stbox, GiST/SP-GiST support, …) sees
 * the correct type tag after the cast.
 * Casts are ASSIGNMENT-only so the user must spell out ::th3index /
 * ::tbigint — accidental int64 reinterpretation surfaces as a clear
 * type-mismatch error.
 ******************************************************************************/
