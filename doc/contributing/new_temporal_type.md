...
It captures four things that don't fit in the API reference or the user manual:
...
4. **The production-readiness essay** — the hazards, conventions, and DocBook expectations that turn a working type into a type production workloads can rely on.

Read all four before starting. The walkthrough sequences the work; the runbook is the per-step reference; the parity checklist is the pre-merge contract; the production-readiness essay is what turns a merged PR into something a workload can rely on.
...
Sections 1, 3, 4, and 5 of this document are reference material. **This section is the path through them.** Read it before opening a single file.
...
| 13–14 | Parity checklist (§4) + production-readiness pass (§5) | Every row in the parity table is checked. The DocBook chapter has a `<sect1 xml:id="foo_production_guidance">` covering the four-to-six named hazards specific to this type. |
...
3. **The production-readiness story.** The named hazards specific to your type. The 2D-vs-3D rationale. The when-to-use-this-vs-the-peer-type guidance. This requires understanding the value semantics deeply enough to know what *can* go wrong.
...
- Step interpolation + exclusive upper bound requires the last instant's value to equal the previous one. Either copy the previous value (cbuffer pattern) or pin `upper_inc = true` (simpler for opaque value types).
...
- `data/load_foo.sql.in` — `CREATE EXTENSION foo` + `CREATE EXTENSION mobilitydb_datagen CASCADE` + `\i @CMAKE_SOURCE_DIR@/mobilitydb/datagen/foo/create_test_tables_foo.sql` + `setseed(0.5)` + `SELECT create_test_tables_foo(100)`. `configure_file` to `data/load_foo.sql` in the build dir so `@CMAKE_SOURCE_DIR@` is substituted.
...
| DocBook *production-guidance* sect1 | `<sect1 xml:id="foo_production_guidance">` covering when-to-use, named hazards, durability, design-restriction rationale | pose, tcbuffer |
...
## 5. The production-readiness essay

A type that passes the parity checklist still isn't production-ready. Production-readiness is the answer to: *if a workload runs this type at scale tomorrow, will it surface a class of bug that was missed because the test corpus didn't reach it?*

The pattern is: **enumerate the named numerical hazards specific to this type, mitigate each one, document each one in a DocBook `production_guidance` sect1.** The pose chapter (`doc/temporal_poses.xml`, sect1 `pose_production_guidance`) and the cbuffer chapter (`doc/temporal_circular_buffers.xml`, sect1 `cbuffer_production_guidance`) are the templates.
...
For each hazard you keep, the DocBook section gets one `<listitem>`:
...
Production-readiness includes telling consumers *when not to use* the type. Every temporal type has at least one peer it overlaps with (`tcbuffer` ⊃ `tgeompoint` at zero radius; `tpose` ⊃ `tgeompoint` at identity orientation). The DocBook section spells out the choice:
...
- Recent end-to-end examples to study commit-by-commit:
  - **`pose` / `tpose`** — production-readiness branch (PRs #810, #846 + the `pose_production_guidance` DocBook sect1).
  - **`trgeometry`** — parity sweep (`feat/trgeo-distance` family + the trgeo DocBook chapter).
  - **`tcbuffer`** — parity + production-readiness pass ([PR #851](https://github.com/MobilityDB/MobilityDB/pull/851)).
...
*Last revisited against commits up to and including the `tcbuffer` parity + production-readiness pass.* When the next type integration lands, refresh the dates and the reference list above.
