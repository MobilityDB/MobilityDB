...
process holding no MEOS library consumes bit-exactly through commodity Apache
Arrow and Parquet tooling. The document gives the exact recipe a reader runs
locally to reproduce that result.
...
MobilityDuck consumer lane (the `temporalFooter()` footer convention and the
native-scalar sidecar quickstart recipe). This document cross-references that
lane; it does not prove it here.

MEOS-WKB is the zero-copy base: in MobilityDB the on-disk varlena is the
in-memory image, so MEOS-WKB is the portable zero-copy form. The optional Arrow
layer is strictly additive and does not change it.
...
no MEOS imports every array through `pyarrow.Array._import_from_c` with no error
and writes real multi-row-group Parquet with per-row-group statistics. A fully
separate process, proven to map no libmeos or
...
the temporal full-surface conversion and the end-to-end demonstration, the set,
span, and spanset conversion, and the tbox and stbox conversion, together with
the per-type external-consumer validators. The data-lake substrate and its
native-scalar sidecar pruning live in the MobilityDuck consumer lane and are
cross-referenced, not reproduced here.
