...
This is `tools/scripts/check_sql_doc_alignment.py`.
    ...
    for path in root.rglob('*.in.sql'):
        ...
        while i < len(lines):
            ...
            if m:
                ...
                # Scan until the next CREATE or semicolon-terminated line to
                # avoid bleeding STRICT from the following function into this
                # one's block when the declarations are close together.
                block_lines = []
                for bi in range(i, min(i + 30, len(lines))):
                    bl = lines[bi]
                    block_lines.append(bl)
                    if bi > i and RE_SQL_CREATE.match(bl):
                        break  # stop at next CREATE (don't include it)
                    if bl.rstrip().endswith(';') and bi > i:
                        break  # stop after closing semicolon
                block = '\n'.join(block_lines)
...
def _function_body_text(lines, body_start):
    """Return the C function body that begins at `lines[body_start]`.

    Brace-track from the body start so PG_ARGISNULL from the *next*
    function is not falsely attributed to this one (a fixed-size line
    window bleeds over into the following definition).
    """
    brace_depth = 0
    in_body = False
    body_lines = []
    for k in range(body_start, min(body_start + 300, len(lines))):
        bl = lines[k]
...
def parse_c_files(src_root: Path):
    """Return a dict mapping c_name to its wrapper metadata.

    Each value is {file, line, ingroup, sqlfn, body_text}, found by
    looking forward from PG_FUNCTION_INFO_V1 for the Doxygen block and
    ...
    for path in src_root.rglob('*.c'):
        ...
        for i, line in enumerate(lines):
            ...
            if body_start is not None:
                body_text = _function_body_text(lines, body_start)
