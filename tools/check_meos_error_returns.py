...
# A function definition in this codebase puts the function name at column 0
# (the return type sits on the preceding line), e.g. `gbox_out(const GBOX ...`.
# The enclosing-function name is a STABLE site key: it survives line-number
# shifts from unrelated edits elsewhere in the file, unlike a raw line number.
_FUNC_DEF_RE = re.compile(r"^([A-Za-z_]\w*)\s*\(")
_NOT_FUNC = {"if", "for", "while", "switch", "return", "sizeof", "else", "do"}


def _enclosing_function(lines, idx: int) -> str:
    """Return the name of the function enclosing line index `idx`."""
    for i in range(idx, -1, -1):
        m = _FUNC_DEF_RE.match(lines[i])
        if m and m.group(1) not in _NOT_FUNC:
            return m.group(1)
    return "?"


def _scan_file(path: Path) -> list[tuple[Path, int, int, str, str]]:
    """Return the potential fall-through violations found in `path`.

    Each list item is a (path, start_line_1based, next_line_1based,
    next_stripped, enclosing_function) tuple for a site that looks like a
    potential fall-through violation.
    """
    with path.open() as f:
        lines = f.readlines()
    ...
    for start, end in _meos_error_calls(lines):
        ...
        if not is_safe:
            func = _enclosing_function(lines, start)
            out.append((path, start + 1, nidx + 1, nstr[:80], func))
    return out


def scan_repo(root: Path) -> list[tuple[Path, int, int, str, str]]:
...
def _site_key(fp: Path, func: str, nstr: str) -> tuple[str, str, str]:
    """Return the line-number-independent baseline key for a site."""
    return (str(fp.relative_to(REPO_ROOT)), func, nstr.replace("\t", " "))


def dump_baseline(findings, path: Path) -> None:
    header = (
        "# meos_error(ERROR, ...) -> non-control-transfer follow-up baseline.\n"
        "# Tab-separated <relative path> <enclosing function> <next statement>,\n"
        "# keyed by function rather than line number so it survives unrelated\n"
    ...
    keys = sorted({_site_key(fp, func, nstr) for fp, _s, _n, nstr, func in findings})
    lines = [header] + [f"{p}\t{fn}\t{ns}\n" for p, fn, ns in keys]
...
def main() -> int:
    ...
    new_sites = [
        (fp, sln, nln, nstr, func)
        for fp, sln, nln, nstr, func in findings
        if _site_key(fp, func, nstr) not in baseline
    ]
    removed = sorted(
        baseline - {_site_key(fp, func, nstr) for fp, _s, _n, nstr, func in findings}
    )
    ...
    if new_sites:
        ...
        for fp, sln, nln, nstr, func in new_sites:
            print(
                f"  {fp.relative_to(REPO_ROOT)}:{sln} (in {func})  next stmt @ "
                f"line {nln}: {nstr}"
            )
        print(
            "\nSee the meos_error doc-comment in meos/include/meos.h. If this "
            "is intentional (a safe-by-design pattern the scanner does not "
            "recognise), regenerate the baseline with:\n"
            "  python3 tools/check_meos_error_returns.py --rebaseline\n"
            "and include the diff in the change."
        )
        return 1
